...
export default function ({ fromDate, toDate, supplier, supplierList, passPrintToElectron }) {
    ...
    const setData = async () => {
        try {
            let url = `${BaseAPI}/reports/stock/category-wise?`;
            ...
            <div className=" hidden">
                <div ref={categoryWise}>
                    <div style={{ textAlign: 'center' }}>
                        <HeaderInfo title={`Category Wise Stock Report`} />
                        {
                            supplier && <h4>Suppiler Name: {supplierList.find(i => i.value == supplier)?.label}</h4>
                        }
                        <div style={{ position: 'relative', margin: 'auto', width: '70%', borderRadius: '5px', padding: '2px', border: '1px solid #8c8c8c', marginTop: '20px', }}>
                            <table style={{ width: '100%', fontSize: '12px' }} >
                                <thead style={{ backgroundColor: 'black', color: '#EFEFEF' }} >
                                    <tr style={{ fontSize: '13px', textTransform: 'uppercase' }}>
                                        <th style={{ padding: '2px' }}>Category Name</th>
