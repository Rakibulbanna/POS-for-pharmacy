...
export default function ({ fromDate, toDate }) {
  ...
  useEffect(() => {
    let url = `${BaseAPI}/promotions/bxgx?`;
    ...
    HTTP.get(url).then(res => {
      ...
      setBuyOneGetOneProducts(() => res.data.data)
    }).catch(err => {
      console.log(err);
    })
  }, [fromDate, toDate]);

  // console.log({ buyOneGetOneProducts })

  return (
    <>
      <HeaderInfo title={"Buy One Get One Summary Reports"} />
      <div style={{ border: '1px solid #8c8c8c', padding: '0.1rem', fontFamily: 'Arial' }}>
        <table style={{ fontSize: '12px', width: '100%', textAlign: 'center' }} >
          ...
          <tbody>
            {buyOneGetOneProducts.map((discount, index) => (
